...
<section class="max-w-4xl p-6 mx-auto bg-indigo-600 rounded-md shadow-md dark:bg-gray-800 mt-20">
    <h1 class="text-xl font-bold text-white capitalize dark:text-white">Laporan Peminjaman</h1>
    <form action="{{ route('laporan.index')}}" enctype="multipart/form-data" method="POST"> <!-- Perbarui method menjadi POST -->
        @csrf
        @method('GET') <!-- Tambahkan method PUT -->
        @if ($errors->any())
        ...
        <div class="flex justify-end mt-6 gap-6">
            <button type="" 
                class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-indigo-600 rounded-md hover:bg-indigo-400 focus:outline-none focus:bg-gray-600" onclick="printPage()">Print</button>
                <a href="{{ route('laporan.index') }}" class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-pink-500 rounded-md hover:bg-pink-700 focus:outline-none focus:bg-gray-600">
                    Back
                </a>
                <script>
                    function printPage() {
