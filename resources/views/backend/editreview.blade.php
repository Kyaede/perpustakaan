...
<section class="max-w-4xl p-6 mx-auto bg-indigo-600 rounded-md shadow-md dark:bg-gray-800 mt-20">
    ...
    <form action="{{ route('ulasan.update', $review->ulasan_id)}}" enctype="multipart/form-data" method="POST"> <!-- Perbarui method menjadi POST -->
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <select id="user_id" name="user_id" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ $user->id == $review->user_id ? 'selected' : '' }}>{{ $user->username }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select id="buku_id" name="buku_id" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                    @foreach ($buku as $item)
                        <option value="{{ $item->buku_id }}" {{ $item->buku_id == $review->buku_id ? 'selected' : '' }}>{{ $item->judul }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input id="nama_kategori" type="text" name="review" value="{{ old('review') ?? $review->review }}"
            ...
            <div>
                ...
                <input id="nama_kategori" type="text" name="rating" value="{{ old('rating') ?? $review->rating }}"
                    class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
            </div>
        ...
        <div class="flex justify-end mt-6 gap-6">
            <button type="submit" 
                ...
                <a href="{{ route('ulasan.index') }}" class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-pink-500 rounded-md hover:bg-pink-700 focus:outline-none focus:bg-gray-600">
                    Back
                </a>
        </div>
    </form>
